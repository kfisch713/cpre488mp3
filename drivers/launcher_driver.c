 ...
 *
 * This driver is based on the 2.6.3 version of drivers/usb/usb-eton.c
...
static int do_read_io(struct usb *dev, size_t count)
{
	int rv;
...
static ssize_t read(struct file *file, char *buffer, size_t count,
			 loff_t *ppos)
{
	struct usb *dev;
	int rv;
...
{
	struct usb *dev = usb_get_intfdata(intf);
